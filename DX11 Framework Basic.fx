...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	float3 lightLecNorm = normalize(lights[0].LightVecW);
	// Compute Colour
	...
	specular += specularAmount * (surface.SpecularMtrl * lights[0].SpecularLight).rgb;
	diffuse += diffuseAmount * (surface.DiffuseMtrl * lights[0].DiffuseLight).rgb;
	ambient += (surface.AmbientMtrl * lights[0].AmbientLight).rgb;
