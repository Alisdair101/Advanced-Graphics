...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	wtMat[1] = normalize(mul(float4(cross(input.NormL, input.Tangent), 0.0f), World).xyz); // Binormal basis vector
	wtMat[2] = normalize(mul(float4(input.NormL, 0.0f), World).xyz); // Normal basis vector

	//// Transform vertex position from model coordinates to world coordinates
	float3 wPos = mul(float4(input.PosL, 1.0f), World).xyz;

	// Compute light and eye vectors in world space
	float3 wLightVect = (lights[0].LightVecW - wPos).xyz; // Light vector in world space 
	...
	output.lightVec = normalize(mul(wtMat, wLightVect));
	output.eyeVec = normalize(mul(wtMat, wEyeVect));
...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	// Renormalize eyeVec and lightVec
